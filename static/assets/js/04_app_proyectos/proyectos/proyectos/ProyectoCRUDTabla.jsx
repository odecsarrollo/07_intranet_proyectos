...
const Tabla = memo(props => {
    const [tipo_proyecto, setTipoProyecto] = useState(null);
    const {
        updateItem,
        singular_name,
        onDelete,
    ...
    let data = _.orderBy(list, ['abierto', 'id_proyecto'], ['desc', 'desc']);
    if (tipo_proyecto) {
        data = data.filter(d => d.id_proyecto.substring(0, 2) === tipo_proyecto)
    }
    const tipos_proyectos_base = _.map(list, p => p.id_proyecto.substring(0, 2)).filter((v, i, self) => self.indexOf(v) === i);

    const tipos_proyectos = tipos_proyectos_base.map(t => ({tipo: t}));
                ...
                tipos_proyectos.map(
                    t =>
                        <span className={`puntero`}
                              ...
                              onClick={() => setTipoProyecto(t.tipo)}
                              key={t.tipo}>[{t.tipo}] </span>
                )
            }
                <span className='puntero'
                      onClick={() => setTipoProyecto(null)}
